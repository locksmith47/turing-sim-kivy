...
import time
import ctypes
from plyer_lach.platforms.win.libs import win_api_defs
from plyer_lach.compat import PY2
...
class WindowsBalloonTip(object):
    ...
    def __init__(self, title, message, app_name, app_icon='', timeout=10):
        ...
        class_name = 'plyer_lachTaskbar' + str(WindowsBalloonTip._get_unique_id())
        if PY2:
        ...
        if class_atom == 0:
            raise Exception('Could not register the plyer_lachTaskbar class.')
